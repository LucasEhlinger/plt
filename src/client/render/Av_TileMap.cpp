#include "Av_TileMap.h"

using namespace render;

bool Av_TileMap::load(const std::string &tileset, sf::Vector2u tileSize, unsigned int *level, unsigned int nb_col,
                   unsigned int nb_row) {
    // load the tileset texture
    if (!m_av_tile.loadFromFile(tileset))
        return false;
...
void Av_TileMap::draw(sf::RenderTarget &target, sf::RenderStates states) const {
    // apply the transform
    states.transform *= getTransform();

    // apply the tileset texture
    states.texture = &m_av_tile;
